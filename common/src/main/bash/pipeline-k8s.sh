...
function performGreenDeploymentOfOtherServices {
	...
	local releaseNote=helm install ${helmoptions} --namespace  "${PAAS_NAMESPACE}" ./otherservices
...
function performGreenDeploymentOfConfigServer() {
	...
	local releaseNote= helm install  --set configserver.image.name="${DOCKER_REGISTRY_ORGANIZATION}/${appName}:${version}" --set configserver.version="${version}"  --namespace  "${PAAS_NAMESPACE}" ./config-server
