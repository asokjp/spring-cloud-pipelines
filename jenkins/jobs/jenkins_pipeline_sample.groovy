...
//Starting of Config server
	...
	if (rollbackStep) {
		dsl.job("${projectNameForConfig}-test-env-rollback-deploy") {
			deliveryPipelineConfiguration('Config-Server-Test', 'Deploy to test latest prod version')
			wrappers {
				deliveryPipelineVersion('${ENV,var="PIPELINE_VERSION"}', true)
	...
	dsl.job("${projectNameForConfig}-prod-env-rollback") {
		...
		""")
			shell('''#!/bin/bash
		${WORKSPACE}/.git/tools/common/src/main/bash/prod_rollback_configserver.sh
		''')
		}
	}
	...
	dsl.job("${projectNameForConfig}-prod-env-complete") {
		...
		""")
			shell('''#!/bin/bash
		${WORKSPACE}/.git/tools/common/src/main/bash/prod_complete_configserver.sh
		''')
		}
	}	
...
class PipelineDefaults {
	...
	private Map<String, String> defaultEnvVars(Map<String, String> variables) {
		...
		setIfPresent(envs, variables, "REPOS")
